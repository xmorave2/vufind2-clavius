 ...
 * @link     http://vufind.org   Main Site
 */
class VudlController extends Zend_Controller_Action
{
    ...
    {
        // GET CONFIGURATION
        $this->configArray = VF_Config_Reader::getConfig();
    }
    ...
    { 
        ...
        // GET XML FILE NAME
        $driver = VF_Search_Solr_Results::getRecord($id);
        ...
        foreach ($xml->xpath('//METS:structMap/METS:div/METS:div') as $div) {
            foreach ($div->xpath('METS:div') as $item) {
                ...
                if ($index < 5 && $div['TYPE'] == 'page_level') {
                    $firstPages[$index] = array(
                    ...
                    foreach ($item->xpath('METS:fptr') as $id) {    
                        ...
                        $firstPages[$index][$file['use']] = $file['src'];
                    }
        ...
        // SEND THE DATA FOR THE FIRST FIVE PAGES
        // (Original, Large, Medium, Thumbnail srcs) and THE DOCUMENTS        
        $this->view->firstPages = $firstPages;
        $this->view->docs = $docs;
    }
     ...
     */
    private function _getAllFiles($xml)
    {
        $files = array();
        // really only one:
     ...
     */
    private function _getDocSummary($xml)
    {
        $data = array();
