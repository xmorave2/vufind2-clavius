...
class SearchController extends VF_Controller_Search
{
    /**
        ...
        if ($savedSearch) {
            ...
            if (isset($filters['publishDate'])) {
                foreach ($filters['publishDate'] as $current) {
                    if ($range = VF_Solr_Utils::parseRange($current)) {
    ...
    protected function getAdvancedFacets()
    {
        ...
        if (!($results = $cache->load('solrSearchHomeFacets'))) {
            ...
            $cache->save($results, 'solrSearchHomeFacets');
        }
        return $results;
    }
