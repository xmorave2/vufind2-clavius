...
class SolrUtilsTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testCapitalizeBooleans()
    {
        ...
        foreach ($tests as $current) {
            $this->assertEquals(
                VF_Solr_Utils::capitalizeBooleans($current[0]), $current[1]
            );
        }
    }
    ...
    public function testCapitalizeRanges()
    {
        ...
        foreach ($tests as $current) {
            $this->assertEquals(
                VF_Solr_Utils::capitalizeRanges($current[0]), $current[1]
            );
        }
    }
    ...
    {
        // basic range test:
        $result = VF_Solr_Utils::parseRange("[1 TO 100]");
        ...
        // test invalid ranges:
        $this->assertFalse(VF_Solr_Utils::parseRange('1 TO 100'));
        $this->assertFalse(VF_Solr_Utils::parseRange('[not a range to me]'));
    }
}