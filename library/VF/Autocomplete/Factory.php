...
class VF_Autocomplete_Factory
{
    ...
    public static function initAutocomplete($module, $params)
    {
        ...
        if (!empty($module)) {
            $module = 'VF_Autocomplete_'.$module;
     ...
     * @param string                           $typeParam  Request parameter
     * containing search type
     * @param string                           $queryParam Request parameter
    ...
    public static function getSuggestions($request, $typeParam = 'type',
        ...
        $type = $request->getParam($typeParam, '');
        $query = $request->getParam($queryParam, '');

        // get Autocomplete_Type config
        $searcher = $request->getParam('searcher', 'Solr');
        $options = VF_Search_Options::getInstance($searcher);
        $config = VF_Config_Reader::getConfig($options->getSearchIni());
