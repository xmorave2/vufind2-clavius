...
class VF_Config_Reader
{
     ...
     * @return string
     */
    public static function getBaseConfigPath($filename, $path = 'application/configs')
    {
        return realpath(APPLICATION_PATH . '/../' . $path) . '/' . $filename;
    }
     ...
     * @return string
     */
    public static function getConfigPath($filename, $path = 'application/configs')
    {
    ...
    public static function loadConfigFile($filename)
    {
        ...
        do {
            $configs[] = new Zend_Config_Ini(
                // Use ASCII 0 as a nest separator; otherwise some of the unusual key
                // names we have (i.e. in WorldCat.ini search options) will get
                // parsed in unexpected ways.
                $fullpath, null,
                array('nestSeparator' => chr(0), 'allowModifications' => true)
            );
