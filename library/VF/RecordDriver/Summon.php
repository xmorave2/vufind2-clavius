 ...
 * @link     http://vufind.org/wiki/other_than_marc Wiki
 */
class VF_RecordDriver_Summon extends VF_RecordDriver_SolrDefault
{
    /**
     * Constructor.
     *
    ...
    public function __construct($data)
    {
        ...
        $searchSettings = VF_Config_Reader::getConfig('Summon');
            ...
            foreach ($this->fields['PublicationDate_xml'] as $current) {
                if (isset($current['month']) && isset($current['year'])) {
                    ...
                    $dates[] = $converter->convertToDisplayDate(
                        'm-d-Y', "{$current['month']}-{$current['day']}-{$current['year']}"
                    );
    ...
    public function getURLs()
    {
        if (isset($this->fields['link'])) {
            return array(
                $this->fields['link'] => VF_Translator::translate('Get full text')
            );
        }
    ...
    public function getContainerReference()
    {
        ...
        if (!empty($vol)) {
            $str .= VF_Translator::translate('citation_volume_abbrev')
                . ' ' . $vol;
        }
        ...
        if (!empty($no)) {
            ...
            $str .= VF_Translator::translate('citation_issue_abbrev')
                . ' ' . $no;
        }
        ...
        if (!empty($start)) {
            ...
            if ($start == $end) {
                $str .= VF_Translator::translate('citation_singlepage_abbrev')
                    . ' ' . $start;
            } else {
                $str .= VF_Translator::translate('citation_multipage_abbrev')
                    . ' ' . $start . ' - ' . $end;
            }
        }
        return $str;
