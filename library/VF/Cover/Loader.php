...
class VF_Cover_Loader
{
    ...
    protected function fetchFromISBN()
    {
        ...
        $isbn = new VF_Code_ISBN($this->isn);
        if ($isbn->get13()) {
     ...
     *
     * @param string $path    Base path of file to find.
    ...
    {
        // Take theme inheritance into account -- iterate down the list of themes
        // from config.ini and check each one in turn for icon images:
        ...
        while (!empty($currentTheme)) {
            ...
            foreach ($formats as $format) {
                ...
                $file
                    = APPLICATION_PATH . "/themes/$currentTheme/" . $path . $format;
