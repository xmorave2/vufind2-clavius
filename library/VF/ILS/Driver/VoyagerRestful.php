 ...
 * @link     http://vufind.org/wiki/building_an_ils_driver Wiki
 */
class VF_ILS_Driver_VoyagerRestful extends VF_ILS_Driver_Voyager
{
    protected $ws_host;
    protected $ws_port;
    protected $ws_app;
    ...
    protected function processHoldingData($data, $patron = false)
    {
        ...
        $mode = VF_ILS_Connection::getHoldsMode();
    ...
    public function checkRequestIsValid($id, $data, $patron)
    {
        ...
        $mode = ("title" == $level) ? VF_ILS_Connection::getTitleHoldsMode()
            : VF_ILS_Connection::getHoldsMode();
    ...
    protected function makeRequest($hierarchy, $params = false, $mode = "GET",
        ...
        $result = $client->request($mode);
        if ($result->isError()) {
            throw new VF_Exception_ILS('Problem with RESTful API.');
        }
    ...
    public function placeHold($holdDetails)
    {
        ...
        try {
            ...
            if (!is_numeric($checkTime)) {
                throw new VF_Exception_Date('Result should be numeric');
            }
        } catch (VF_Exception_Date $e) {
            throw new VF_Exception_ILS('Problem parsing required by date.');
        }
