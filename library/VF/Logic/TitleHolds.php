...
class VF_Logic_TitleHolds
{
     ...
     * Constructor
     *
     * @param VF_ILS_Connection $catalog A catalog connection
     */
    ...
    public function getHold($id)
    {
        // Get Holdings Data
        if ($this->catalog) {
            $mode = VF_ILS_Connection::getTitleHoldsMode();
            if ($mode == "disabled") {
                 return false;
            } else if ($mode == "driver") {
    ...
    {
        // Generate HMAC
        $HMACkey = VF_Crypt_HMAC::generate($HMACKeys, $data);
