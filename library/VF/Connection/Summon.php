 ...
 * @link     http://api.summon.serialssolutions.com/help/api/ API Documentation
 */
class VF_Connection_Summon extends SerialsSolutions_Summon_Zend
{
    /**
    ...
    {
        $config = VF_Config_Reader::getConfig('Summon');
        ...
        if (!isset($options['debug'])) {
            $options['debug'] = VF_Logger::debugNeeded();
        }

        $timeout = isset($config->General->timeout)
            ? $config->General->timeout : 30;
        parent::__construct(
            $apiId, $apiKey, $options, new VF_Http_Client(
