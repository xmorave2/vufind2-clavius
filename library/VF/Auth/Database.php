 ...
 * @link     http://vufind.org/wiki/building_an_authentication_handler Wiki
 */
class VF_Auth_Database extends VF_Auth_Base
{
    protected $username;
    protected $password;
    ...
    public function create($request)
    {
        ...
        if (trim($params['username']) == '') {
            throw new VF_Exception_Auth('Username cannot be blank');
        }
        // Needs a password
        if (trim($params['password']) == '') {
            throw new VF_Exception_Auth('Password cannot be blank');
        }
        // Passwords don't match
        if ($params['password'] != $params['password2']) {
            throw new VF_Exception_Auth('Passwords do not match');
        }
        // Invalid Email Check
        $validator = new Zend_Validate_EmailAddress();
        if (!$validator->isValid($params['email'])) {
            throw new VF_Exception_Auth('Email address is invalid');
        }

        // Make sure we have a unique username
        $table = new VuFind_Model_Db_User();
        if ($table->getByUsername($params['username'], false)) {
            throw new VF_Exception_Auth('That username is already taken');
        }
        // Make sure we have a unique email
        if ($table->getByEmail($params['email'])) {
            throw new VF_Exception_Auth('That email address is already used');
        }
