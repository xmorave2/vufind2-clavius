...
class VF_Logger extends Zend_Log
{
    ...
    protected function addWriters($writer, $filters)
    {
        ...
        foreach ($filters as $filter) {
            ...
            case 'debug':
                ...
                $max = Zend_Log::INFO;  // Informational: informational messages
                $min = Zend_Log::DEBUG; // Debug: debug messages
                break;
            case 'notice':
                $max = Zend_Log::WARN;  // Warning: warning conditions
                $min = Zend_Log::NOTICE;// Notice: normal but significant condition
                break;
            case 'error':
                $max = Zend_Log::CRIT;  // Critical: critical conditions
                $min = Zend_Log::ERR;   // Error: error conditions
                break;
            case 'alert':
                $max = Zend_Log::EMERG; // Emergency: system is unusable
                $min = Zend_Log::ALERT; // Alert: action must be taken immediately
                break;
            ...
            $filter1 = new Zend_Log_Filter_Priority($min, '<=');
            $filter2 = new Zend_Log_Filter_Priority($max, '>=');
    ...
    {
        Zend_Registry::get('Log')->log($msg, Zend_Log::DEBUG);
    }
